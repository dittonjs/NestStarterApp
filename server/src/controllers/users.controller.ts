...
export class UsersController {
  ...
  async getCurrentUser(@JwtBody() jwtBody: JwtBodyDto) {
    const user = await this.prisma.user.findUnique({where: { id: jwtBody.userId }});
    return { user };
  }
  ...
  async create(@Body() userPayload: CreateUserDto, @Res({ passthrough: true }) res: Response) {
    const role = await this.prisma.role.findUnique({ where: { roleKey: RoleKey.USER }});
    try {
      ...
      const refreshJwtToken = this.jwtService.issueRefreshToken({ id: user.refreshTokens[0].id, userId: user.id });
